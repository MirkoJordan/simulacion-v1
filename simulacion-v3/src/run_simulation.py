...
CITIES = {
    "Madrid": {
        ...
        "station_id": "LEMD",
        "city_slug": "madrid",
        "cache_file": "lemd_historical.csv"
    },
    "London": {
        "lat": 51.505, 
        "lon": 0.055, 
...
MIN_PROB = 0.25
MIN_EDGE = 0.08  # Optimized V3 strategy
MAX_OPTS = 2
...
def download_ground_truth_iem(station_id, past_days, tz, cache_file=None):
    ...
    if cache_file:
        # Look for cache files in root data folder
    ...
    if start_download < end:
        ...
        try:
            ...
            try:
                ...
                print(f"   [⚠️ ERROR CRÍTICO] Ambos servidores fallaron: {e2}")
                df_downloaded = pd.DataFrame()
...
def download_forecasts(lat, lon, past_days, tz, target_date=None):
    local_tz = pytz.timezone(tz)
    if target_date is None:
        target_date = datetime.now(local_tz)
        
    today_date = datetime.now(local_tz).date()
    is_today = (target_date.date() == today_date)
    ...
    if is_today:
        ...
        try:
            ...
            print(f"   [Aviso Forecast] Error al descargar pronostico en vivo: {e}")
            return pd.DataFrame()
    else:
        ...
        try:
            ...
            df_hourly = pd.DataFrame({
                ...
                "Temp_Predicha": r_single["hourly"]["temperature_2m"],
                "Humidity_Predicha": r_single["hourly"]["relative_humidity_2m"],
                "Cloud_Predicha": r_single["hourly"]["cloud_cover"],
                "Pressure_Predicha": r_single["hourly"]["surface_pressure"],
                "Wind_Speed_Predicha": r_single["hourly"]["wind_speed_10m"],
                "Wind_Dir_Predicha": r_single["hourly"]["wind_direction_10m"]
            })
            df_hourly["Fecha"] = df_hourly["Fecha_Hora"].dt.date
            ...
            print(f"   [Aviso Forecast] Error al descargar pronostico de la corrida {run_str}: {e}")
            return pd.DataFrame()

def download_historical_bulk_forecasts(lat, lon, past_days, tz):
    ...
    try:
        ...
        print(f"   [Aviso Forecast Bulk] Error en descarga masiva: {e}")
        return pd.DataFrame()

def build_dataset(city_name, config, target_date=None):
    ...
    df_fcst_bulk = download_historical_bulk_forecasts(config["lat"], config["lon"], 1100, config["timezone"])
    
    if target_date is not None:
        df_fcst_target = download_forecasts(config["lat"], config["lon"], 1100, config["timezone"], target_date=target_date)
        if not df_fcst_target.empty:
            df_fcst = pd.concat([df_fcst_bulk, df_fcst_target]).drop_duplicates(subset=["Fecha"], keep="last")
        else:
            df_fcst = df_fcst_bulk
    else:
        df_fcst = df_fcst_bulk
    ...
    # Corrected boolean operator precedence in pandas
    df = df[(df["Fecha"] < today_date) | (df["Temp_Max_Real"].notna())]
    return df
...
def main():
    # Target docs path for simulation state
    state_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "docs", "simulacion-v3", "data", "simulation_state.json")
    ...
    print(f"======================= SIMULADOR CLOUD V3 ({today_str}) =======================")
    ...
    for city, config in CITIES.items():
        ...
        try:
            ...
            df_all = build_dataset(city, config, target_date=target_date_dt)
        ...
        for bot_id, bot_data in models.items():
            ...
            for m in markets:
                ...
                # Minimum probability (25%) and Optimized V3 Minimum Edge (8%)
            ...
            if choices:
                ...
                for c in choices:
                    ...
                    print(f"   - {state['bots'][bot_id]['name']}: Apostó ${money_per_bet:.2f} a '{c['option']}' (Precio: ${effective_price:.3f}, IA: {c['prob']*100:.1f}%, Satélite: {today_row['Temp_Max_Predicha'].values[0]:.2f}°C, Pred_IA: {pred_ia:.2f}°C, {val_err_str})")
            else:
    ...
    print("\n[+] Simulación diaria V3 completada con éxito y balances guardados.")
    print("=" * 77)
